...
def loss_function(melspec_hat, melspec, cls_z_logits, cls_z_prob, z_dist, cls_z_prob_2,
    ...
    if not step is None:
        ...
        beta_0 = 1
    beta_1 = min(step / 200 * beta, beta)
    
    recon_loss = torch.nn.MSELoss()(melspec_hat, melspec)

    if is_sup:
        class_weights = torch.Tensor([2.0, 1.0, 1.0, 2.0]).cuda()
        cls_loss = torch.nn.CrossEntropyLoss(weight=class_weights)(cls_z_prob, emotion_cls)
        ...
        loss = 8 * recon_loss + cls_loss + beta_0 * kl_loss

        # cyclic loss
        class_weights = torch.Tensor([2.0, 0.5, 1.0, 2.0]).cuda()
        loss2 = beta_1 * torch.nn.CrossEntropyLoss(weight=class_weights)(cls_z_prob_2, emotion_cls)
        clf_acc_2 = accuracy_score(torch.argmax(cls_z_prob_2, dim=-1).cpu().detach().numpy(),
                                    emotion_cls.cpu().detach().numpy())

        return loss, loss2, recon_loss, cls_loss, kl_loss, clf_acc, clf_acc_2
...
def training():
    ...
    for ep in range(1, total_epoch):
        ...
        for i, x in enumerate(train_s_dl):
            ...
            audio, onset_pr, frame_pr, emotion_cls, pedalling = x     # (b, 320000), (b, t=625, 88)
            # pr = torch.cat([onset_pr, frame_pr], dim=-1)
            pr = onset_pr
            melspec = torch.transpose(wav_to_melspec(audio), 1, 2)[:, :-1, :]   # (b, 625, 128)
            ...
            melspec_hat, z, cls_z_logits, cls_z_prob, z_dist, cls_z_prob_2 = model(melspec, pr, pedalling)
            loss, loss2, recon_loss, cls_loss, kl_loss, clf_acc, clf_acc_2 = loss_function(melspec_hat, melspec, cls_z_logits,
                                                                                            cls_z_prob, z_dist,
                                                                                            cls_z_prob_2,
                                                                                            step=step_sup,
            ...
            if ep > 10:
                los = loss + loss2      # try updating encoder as well
                loss.backward()
                optimizer.step()

                # # cyclic loss -- only allow to propogate on decoder
                # for name, param in model.named_parameters():
                #     if "bilstm" in name or "out_conv" in name:
                ...
                # # do a second pass
                # optimizer.zero_grad()
                # melspec_hat, z, cls_z_logits, cls_z_prob, z_dist, cls_z_prob_2 = model(melspec, pr)
                # loss, loss2, recon_loss, cls_loss, kl_loss, clf_acc, clf_acc_2 = loss_function(melspec_hat, melspec, cls_z_logits,
                #                                                                                 cls_z_prob, z_dist,
                #                                                                                 cls_z_prob_2,
                #                                                                                 step=step_sup,
            ...
            print("", end="\r")
            print("Batch {}/{}: Recon: {:.4} KL Sup: {:.4} CLF Loss: {:.4} Acc: {:.4} CLF Loss 2: {:.4} Acc 2: {:.4} ".format(i+1, 
                                                                                                                    len(train_s_dl),
                                                                                                                    recon_loss.item(), 
                                                                                                                    kl_loss.item(),
                                                                                                                    cls_loss.item(), 
            ...
            train_sup_writer.add_scalar('CLF Loss 2', loss2.item(), global_step=step_sup)
            train_sup_writer.add_scalar('CLF Acc 2', clf_acc_2, global_step=step_sup)

            step_sup += 1
            learning_rate_counter +=1
        
        # evaluate supervised
        eval_loss, eval_recon_loss, eval_cls_loss, eval_cls_acc, eval_kl, eval_cls_loss_2, eval_cls_acc_2 = 0, 0, 0, 0, 0, 0, 0
        for i, x in enumerate(val_s_dl):
            ...
            eval_cls_loss_2 += loss2.item() / len(val_s_dl)
            eval_cls_acc_2 += clf_acc_2 / len(val_s_dl)

        print("", end="\r")
        print("Sup Eval: Recon: {:.4} KL Sup: {:.4} CLF Loss: {:.4} Acc: {:.4} CLF Loss 2: {:.4} Acc 2: {:.4}".format(
                                                                                                    eval_recon_loss, 
                                                                                                    eval_kl,
                                                                                                    eval_cls_loss, 
                                                                                                    eval_cls_acc,
        ...
        eval_sup_writer.add_scalar('CLF Loss 2', eval_cls_loss_2, global_step=step_sup)
        eval_sup_writer.add_scalar('CLF Acc 2', eval_cls_acc_2, global_step=step_sup)
        ...
        if ep % 1 == 0:
            # plot spectrograms
            audio, onset_pr, frame_pr, emotion_cls, pedalling = train_s_ds[10]
            ...
            melspec_hat, z, cls_z_logits, cls_z_prob, z_dist, cls_z_prob_2 = model(melspec, pr, torch.Tensor([pedalling]))
            ...
            for i, x_temp in tqdm(enumerate(train_s_dl), total=len(train_s_dl), desc='Running latents on train set:'):
                audio, onset_pr, frame_pr, emotion_cls, pedalling = x_temp
                pr_visualize = onset_pr + frame_pr
                # pr = torch.cat([onset_pr, frame_pr], dim=-1)
                pr = onset_pr
                melspec = torch.transpose(wav_to_melspec(audio), 1, 2)[:, :-1, :]   # (b, 625, 128)
...
if __name__ == "__main__":
    ...
    cls_lst = []
    for i, x in enumerate(train_s_dl):
        audio, onset_pr, frame_pr, cls, pedalling = x     # (b, 320000), (b, t=625, 88)
        cls_lst.append(cls)
