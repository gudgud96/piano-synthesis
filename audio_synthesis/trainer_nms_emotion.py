...
def loss_function(melspec_hat, melspec, cls_z_logits, cls_z_prob, z_dist, is_sup=False, emotion_cls=None,
    ...
    # kl annealing
    if not step is None:
        beta_0 = min(step / 100000 * beta, beta)
    else:
        beta_0 = 1
    
    recon_loss = torch.nn.MSELoss()(melspec_hat, melspec)

    # print(cls_z_prob[0].cpu().detach().numpy())
    # print(cls_z_prob[1].cpu().detach().numpy())
    # print(cls_z_prob[2].cpu().detach().numpy())
    ...
    if is_sup:
        ...
        return loss, recon_loss, cls_loss, kl_loss, clf_acc, beta_0
        ...
        return loss, recon_loss, kl_cls, kl_loss, entropy(cls_z_prob, cls_z_logits).mean(), beta_0
...
def training():
    ...
    for ep in range(1, total_epoch):
        ...
        for i, x in enumerate(train_dl):
            ...
            loss, recon_loss, cls_loss, kl_loss, h_q, beta_0 = loss_function(melspec_hat, melspec, cls_z_logits,
            ...
            train_unsup_writer.add_scalar('Beta', beta_0, global_step=step_unsup)
        ...
        for i, x in enumerate(train_emotion_dl):
            ...
            melspec_hat, z, cls_z_logits, cls_z_prob, z_dist = model(melspec, pr)
            loss, recon_loss, cls_loss, kl_loss, clf_acc, beta_0 = loss_function(melspec_hat, melspec, cls_z_logits,
                                                                        cls_z_prob, z_dist,
                                                                        step=step_sup,
        ...
        for i, x in enumerate(val_emotion_dl):
            ...
            melspec_hat, z, cls_z_logits, cls_z_prob, z_dist = model(melspec, pr)
            loss, recon_loss, cls_loss, kl_loss, clf_acc, _ = loss_function(melspec_hat, melspec, cls_z_logits,
                                                                        cls_z_prob, z_dist,
                                                                        step=step_sup,
                                                                        is_sup=True, emotion_cls=emotion_cls)
        ...
        if ep % 1 == 0:
            # plot spectrograms
            audio, onset_pr, frame_pr, _, emotion_cls = train_emotion_ds[10]
            ...
            train_unsup_writer.add_figure('tsne_z_unsup', fig, global_step=step_sup, close=True)
            ...
            cls_lst = []
            actual_cls_lst = []
            for i, x_temp in tqdm(enumerate(train_emotion_dl), total=len(train_emotion_dl), desc='Running latents on train set:'):
                audio, onset_pr, frame_pr, _, emotion_cls = x_temp
                pr_visualize = onset_pr + frame_pr
                pr = torch.cat([onset_pr, frame_pr], dim=-1)
                melspec = torch.transpose(wav_to_melspec(audio), 1, 2)[:, :-1, :]   # (b, 625, 128)
                melspec_original = wav_to_melspec(audio)
            ...
            fig = plt.figure(figsize=(8,8))
            sns.scatterplot(tsne_features[:,0], tsne_features[:,1], palette=palette, hue=color, legend='full')
            train_unsup_writer.add_figure('tsne_z_sup', fig, global_step=step_sup, close=True)
...
if __name__ == "__main__":
    ...
    train_emotion_dl = DataLoader(train_emotion_ds, batch_size=args["batch_size"] // 4, shuffle=False, num_workers=0)
    ...
    for i, x in enumerate(train_emotion_dl):
        audio, onset_pr, frame_pr, _, emotion_cls = x     # (b, 320000), (b, t=625, 88)
        cls_lst_2.append(emotion_cls)
        
    cls_lst_2 = torch.cat(cls_lst_2, dim=0)
    from collections import Counter
    print("Supervised labels for training:", Counter(cls_lst_2.cpu().numpy()))

    # load writers
    current_time = datetime.datetime.now().strftime('%Y%m%d-%H%M%S')
    train_log_dir = 'logs/'+args['name']+'_emotion_energy/'+current_time+'/train'
    eval_log_dir = 'logs/'+args['name']+'_emotion_energ/'+current_time+'/eval'
