...
# model
version = 2
model = MusicVCNet(embedding_dim=args["hidden_dim"], 
                    vocab_size=388+2, 
                    mel_token_size=106,
...
def loss_function(dec_global_out, performance_tokens_padded,
    ...
    if label.item() == -1:
        ...
        for k in torch.arange(0, n_component):       # number of components
            ...
            kld_lat = torch.mean(kl_divergence(style_dis, dis_pz_y), dim=-1)
            kld_lat *= qy_x[:, k]
        ...
        loss = perf_loss + mel_loss + beta0 * (kld_lat_total + kld_cls)
        return loss, perf_loss, perf_acc, mel_loss, mel_acc, kld_lat_total, kld_cls

    else:
        mu_pz_y, var_pz_y = model.mu_lookup(label.cuda().long()), model.logvar_lookup(label.cuda().long()).exp_()
        dis_pz_y = Normal(mu_pz_y, var_pz_y)
        kld_lat_total = torch.mean(kl_divergence(style_dis, dis_pz_y), dim=-1).mean()

        label_clf_loss = nn.CrossEntropyLoss()(qy_x, label.cuda().long())
        loss = perf_loss + mel_loss + beta0 * (kld_lat_total) + label_clf_loss

        return loss, perf_loss, perf_acc, mel_loss, mel_acc, kld_lat_total, torch.Tensor([0])
...
def training_phase(model, optimizer, scheduler):
    ...
    for i in range(1, args['n_epochs'] + 1):
        ...
        for j, x in enumerate(train_dl_dist):
            ...
            performance_tokens, mel_tokens, labels = x
            performance_tokens = performance_tokens.cuda().long()
            mel_tokens = mel_tokens.cuda().long()
            labels = labels.cuda().long()

            optimizer.zero_grad()
            dec_global_out, dec_mel_out, style_dis, \
                z_style, logLogit_qy_x, qy_x, style_y = model(performance_tokens, mel_tokens)

            z_lst.append(z_style.detach())
            ...
            loss, perf_loss, perf_acc, mel_loss, mel_acc, \
                kld_lat_total, kld_cls = loss_function(dec_global_out, performance_tokens_padded,
                                dec_mel_out, mel_tokens_padded,
                                qy_x, style_dis, logLogit_qy_x,
                                labels, step, beta=args["beta"])

            print("Step: {} Perf: {:.4f} {:.4f} Mel: {:.4f} {:.4f} KLD Latent: {:.4f} KLD cls: {:.4f}".format(
                            step, perf_loss.item(), perf_acc,
                            mel_loss.item(), mel_acc,
                            kld_lat_total.item(), kld_cls.item()), end="\r")
            ...
            train_summary_writer.add_scalar('kld_loss', kld_lat_total.item() + kld_cls.item(), global_step=step)
            train_summary_writer.add_scalar('learning_rate', scheduler.rate(), global_step=step)

            if step % 250 == 0:

                print("Saving model", "params/music-vc-{}.pt".format(version))
                # for model, save the whole thing
                torch.save(single_model, "params/music-vc-{}.pt".format(version))
                torch.save(optimizer.state_dict(), "params/opt-music-vc-{}.pt".format(version))
                torch.save(scheduler.state_dict(), "params/scheduler-music-vc-{}.pt".format(version))
                ...
                for j, x in tqdm(enumerate(val_dl_dist), total=len(val_dl_dist)):
                    ...
                    test_kld += kld_lat_total.item() + kld_cls.item()
                ...
                print("Evaluation - Perf: {:.4f} {:.4f} Mel: {:.4f} {:.4f} KLD Latent: {:.4f} KLD cls: {:.4f}".format(
                            step, perf_loss.item(), perf_acc,
                            mel_loss.item(), mel_acc,
                            kld_lat_total.item(), kld_cls.item()), end="\r")
