'''
Refer code from: https://github.com/jason9693/MusicTransformer-pytorch/blob/master/custom/layers.py
'''
...
class Decoder(torch.nn.Module):
    ...
    def forward(self, x, mask, lookup_mask, training, enc_output=None):
        weights = []
        # adding embedding and position encoding.
        x = self.embedding(x.to(torch.long))  # (batch_size, input_seq_len, d_model)
        x *= math.sqrt(self.d_model)
        ...
        x = self.dropout(x)
        for i in range(self.num_layers):
            x, w1, w2 = self.dec_layers[i](x, enc_output, lookup_mask=lookup_mask, 
                                          mask=mask, training=training, w_out=True)
...
class CommonEncoder(torch.nn.Module):
    def __init__(self, num_layers, d_model, input_vocab_size, rate=0.1, max_len=None):
        ...
        self.encoder_layer = nn.TransformerEncoderLayer(d_model=d_model, nhead=d_model // 64)
...
class CommonDecoder(torch.nn.Module):
    def __init__(self, num_layers, d_model, input_vocab_size, rate=0.1, max_len=None):
