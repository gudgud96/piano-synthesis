...
scheduler = CustomSchedule(args["hidden_dim"], optimizer=optimizer)
...
val_dl_dist = DataLoader(val_ds_dist, batch_size=args["batch_size"], shuffle=is_shuffle, num_workers=0)
...
def training_phase(model, optimizer, scheduler):
    ...
    for i in range(1, args['n_epochs'] + 1):
        ...
        for j, x in tqdm(enumerate(train_dl_dist), total=len(train_dl_dist)):
            ...
            performance_tokens_padded = F.pad(input=performance_tokens[:, 1:], 
            ...
            if step % 250 == 0:
                ...
                print("Saving model", "params/transformer-vae-{}.pt".format(step))
                # for model, save the whole thing
                torch.save(model, "params/transformer-vae-{}.pt".format(step))
                torch.save(optimizer.state_dict(), "params/opt-{}.pt".format(step))
                torch.save(scheduler.state_dict(), "params/scheduler-{}.pt".format(step))
...
def evaluation_phase(model, optimizer, scheduler):
    ...
    # model.load_state_dict(torch.load("params/transformer-vae-4750.pt"))
    ...
    def run(dl):
        ...
        for j, x in enumerate(dl):
            ...
            performance_tokens = x
            performance_tokens = performance_tokens.cuda().long()
            # melody_tokens = melody_tokens.cuda(t).long()
            ...
            # print(performance_tokens_padded[:20])
            # print(torch.argmax(out, dim=-1)[:20])
            # print()
